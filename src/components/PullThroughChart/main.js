...
class pullChart {
    ...
    const text = svg_center.append('text')
      ...
      .style('font-size', '2.0em').style('fill', '#fff');
    ...
    var nodeLinear=d3.scale.linear()
      ...
      const node_g = svg_center.selectAll('nodes').data(chartData.relations).enter().append("g").attr('class', 'nodes')
      .on('click', function(d, i){        
        _this.props.formatData && _this.props.formatData(d.name);
